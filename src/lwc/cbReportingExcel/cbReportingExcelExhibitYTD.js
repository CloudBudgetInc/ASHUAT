...
let _this;
const setContextYTD = (context) => _this = context;
let separatedReportingBalances = {};// reporting balances separated by Dimension 2, key is Dim2 Name
const FIRST_SHEET_NAME = 'Main';
let FILE_NAME = "Budget Exhibit YTD & Projection ";

const manageDataAndGenerateFileYTD = () => {
		...
		try {
			...
			if (rb.c2g__Type__c === 'Actual') {
				...
				if (rb.Year__c === _this.selectedBY) {
					reportLine.actualYTD += parseFloat(invertedIncomeActual);
				} else {
					reportLine.actual += parseFloat(invertedIncomeActual);
				}
			} else if (rb.c2g__Type__c === 'Budget') {
				if (rb.Year__c === _this.selectedBY) {
				...
				reportLine.projectedBudget += parseFloat(rb.c2g__HomeValue__c);
			}
		} catch (e) {
...
const getNewReportLine = (rb, lineKey) => {
	return {
		...
		label: '',
		actual: 0,
		actualYTD: 0, // period closed (additional column requested)
		approvedBudget: 0, // selected BY
		projectedBudget: 0, // projected budget (additional column requested)
...
const getHeaderParams = () => {
	if (!HEADER_PARAMS) {
		...
		HEADER_PARAMS = [
			...
			{title: `${_this.selectedBY} Actual YTD`, width: 15},
			{title: `${_this.selectedBY} Approved Budgeted`, width: 22},
			{title: `${_this.selectedBY} Projected Budgeted`, width: 22},
...
const FIELD_ORDER = ['programProject', 'incomeStatementGroup', 'generalLedgerName', 'accountSubAccount', 'label', 'actual', 'actualYTD', 'approvedBudget', 'projectedBudget', 'processedBudget', 'processedVsApproved', 'processedVsApprovedPercent'];
	...
	try {
		...
		lines.forEach(line => {
			...
			FIELD_ORDER.forEach((f, i) => {
				...
				if (['actual', 'actualYTD', 'approvedBudget', 'projectedBudget', 'processedBudget', 'processedVsApproved'].includes(f)) cell.numFmt = CURRENCY_FMT;
...
export {setContextYTD, manageDataAndGenerateFileYTD}